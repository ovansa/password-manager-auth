...
function verifyLemonSignature(rawBody, signatureHeader) {
    ...
    if (!secret || !signatureHeader)
        return false;
    const digest = Buffer.from(crypto_1.default.createHmac('sha256', secret).update(rawBody).digest('hex'), 'utf8');
...
function getSubscriptionExpiry(payload, plan) {
    if (plan === 'lifetime')
        return null;
    const attrs = payload.data?.attributes ?? {};
    return (toTimestamp(attrs['renews_at']) ||
        toTimestamp(attrs['ends_at']) ||
        toTimestamp(attrs['trial_ends_at']) ||
        firebase_1.admin.firestore.Timestamp.fromDate(new Date(Date.now() +
            ((0, license_1.getLicensePlanDurationDays)(plan) ?? 0) * 24 * 60 * 60 * 1000)));
}
function getLicenseExpiry(plan) {
    const duration = (0, license_1.getLicensePlanDurationDays)(plan);
    if (!duration)
        return null;
    return firebase_1.admin.firestore.Timestamp.fromDate(new Date(Date.now() + duration * 24 * 60 * 60 * 1000));
}
...
router.post('/lemon-squeezy', async (req, res) => {
    ...
    try {
        const result = await firebase_1.db.runTransaction(async (tx) => {
            ...
            if (eventDoc.exists)
                return { alreadyProcessed: true, licenseKey: null };
            const now = firebase_1.admin.firestore.FieldValue.serverTimestamp();
            const attrs = payload.data?.attributes ?? {};
            const sourceId = objectId ?? null;
            const purchaseRef = firebase_1.db.collection('lemon_squeezy_purchases').doc(eventId);
            tx.set(eventRef, {
            ...
            const activeStatus = isSubscriptionEvent
                ...
                : eventName === 'order_refunded' ||
                    ...
                    ? 'expired'
                    : 'active';
            tx.set(firebase_1.db.collection('subscriptions').doc(email), {
                status: activeStatus,
                plan,
        ...
        logger_1.logger.error('lemon.webhook_error', error, { eventName, email, plan });
        res
            .status(500)
